...
// Função cuda que realiza a adição dos vetores na gpu
__global__ void add(int *a, int *b, int *c, int n) {
    // Achando a posição do indíce da thread atual:
...
// Função que realiza a adição dos vetores na cpu
...
int main() {
    // Variável que guarda a dimensão dos nossos vetores
    int n;

    std::cout << "Digite o tamanho desejado para os vetores: " << std::endl;
    std::cin >> n;

    // Declaração dos vetores na cpu e gpu
    int *a, *b, *c;
    int *d_a, *d_b, *d_c;
